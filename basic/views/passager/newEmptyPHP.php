...
<div class="reserv-form">
    ...
    <?= $form->field($model, 'reys')->dropDownList(ArrayHelper::map(app\models\Flight::find()->all(),'reys','reys','city'),['prompt'=>'Select...',
    ...
    <?= $form->field($model, 'class')->dropDownList(ArrayHelper::map(app\models\Place::find()->all(),'class','class'),['prompt'=>'Выберите...',
    ...
    <?= $form->field($model, 'id_place')->dropDownList(ArrayHelper::map(app\models\Place::find()->all(),'id_place','id_place'),['prompt'=>'Select...']) ?>
 
    
    <?= $form->field($model, 'date')->dropDownList(ArrayHelper::map(app\models\Date::find()->all(),'date','date'),['prompt'=>'Select...'])  ?>
...
'])?>            
    ...
    <?= $form->field($model, 'tickettype')->dropDownList(ArrayHelper::map(app\models\Tickettype::find()->all(),'type','type'),['prompt'=>'Select...',
    ...
    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? 'Create1' : 'Update', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
